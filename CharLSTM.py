...
class CharLSTM:
    ...
    def __init__(self, layers, num_possible_characters):
        ...
        self.forward_pass = theano.function([num_steps], [results[0]], updates=updates, allow_input_downcast=True)
        ...
        cost = -T.sum(T.log(prob_correct_v))
    ...
    def train(self, dataset, batch_size=100, max_num_batches=100):
        print("Training...")
        ...
        for batch in range(1, max_num_batches):
            ...
            if batch % 10 == 0:
                print("Batch ", batch)
                pickle.dump(self.model.params, 'batch' + str(batch) + '.p')
                cost = 0
...
def output_to_str(data, letter_number_dict):
    number_letter_dict = dict((v, k) for k, v in map.iteritems())
    s = ''
    for d in data:
...
if __name__ == '__main__':
    ...
    m = CharLSTM([50,100,50], len(lnd))
    m.train(data)
    print(output_to_str(m.forward_pass(1000)[0], lnd))